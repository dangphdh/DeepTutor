...
- Chưa có mục tiêu nào? Hãy thiết kế một lộ trình từ tài liệu của người học (dùng `rag` / `read_source` khi có tài liệu đính kèm) và gọi `mastery_build`. Gắn nhãn mỗi điểm kiến thức: memory (sự kiện cần nhớ), procedure (kỹ năng từng bước), concept (ý tưởng cần hiểu), design (sáng tạo/tư duy mở).
