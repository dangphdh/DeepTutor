...
然后针对该知识点行动：
- 还没有任何知识点？如果 `mastery_status` 已经返回活动状态（已预加载课程），请按其模块/知识点顺序进行——不要重新设计。否则根据学习者的材料设计一条路径（材料已挂载时用 `rag` / `read_source`），调用 `mastery_build`。给每个知识点标类型：memory（记忆/事实）、procedure（程序/步骤技能）、concept（概念/需理解）、design（设计/开放判断）。
